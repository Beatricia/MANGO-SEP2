...
public class AdminDatabaseConnection {
    ...
    public ArrayList<MenuItem> getTopThreeDishes() throws SQLException
    {
        ...
        try(Connection connection = DatabaseConnImp.getConnection())
        {
            String sqlTopDishes = "SELECT orderItem.itemName, sum(orderItem.quantity) as sum FROM orderItem WHERE orderNumber in "
                + "(SELECT orderNumber FROM \"order\" WHERE collected = true) GROUP BY itemName ORDER BY sum DESC LIMIT 3";

            PreparedStatement statement = connection.prepareStatement(sqlTopDishes);
            ResultSet set = statement.executeQuery();
            ...
            while (set.next())
                {
                    ...
                    String sqlIngredients = "SELECT name FROM ingredient WHERE id IN (SELECT ingredientId FROM menuItemIngredient WHERE itemName = '" + itemName + "'";
                    PreparedStatement statementIngredient = connection.prepareStatement(sqlIngredients);
                    ...
                    String sqlGetMenuItem = "SELECT * FROM menuItem WHERE name = '" + itemName + "'";

                    PreparedStatement statement1 = connection.prepareStatement(sqlGetMenuItem);
                    ...
                    while (set.next())
                    {
                        ...
                        MenuItem menuItem = new MenuItem(itemName,ingredients,price,imgPath);
    ...
    public ArrayList<Integer> getNumberOfOrders() throws SQLException
    {
        ...
        try(Connection connection = DatabaseConnImp.getConnection())
        {
            ...
            for (int i = 0; i <5 ; i++)
            {
                String sql = "SELECT count(\"order\") FROM \"order\" WHERE collected = true AND date = '" + date + "'";
